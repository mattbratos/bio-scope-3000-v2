...
export function Timeline({ frames, currentTime, duration, onSeek, videoUrl }: TimelineProps) {
  ...
  const captureFrame = useCallback(async (video: HTMLVideoElement, time: number): Promise<string> => {
      ...
      try {
        ...
        for (const frame of frames) {
          try {
            ...
            console.error(`Error generating thumbnail for frame ${frame.id}:`, error)
          }
        }
      } catch (error) {
            ...
            >
              ...
              <div
                className={`absolute bottom-0 left-0 right-0 h-0.5 bg-green-500 transition-all ${
                  Math.abs(currentTime - frame.timestamp) < 0.1 ? "opacity-100" : "opacity-0"
                }`}
              />
